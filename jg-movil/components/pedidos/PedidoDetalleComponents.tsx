...
// Pedido Header Info
interface PedidoHeaderInfoProps {
  pedido: Pedido;
}

export function PedidoHeaderInfo({ pedido }: PedidoHeaderInfoProps) {
  return (
...
// Productos Table
interface PedidoProductosTableProps {
  detalles: Pedido['detalles'];
}

export function PedidoProductosTable({ detalles }: PedidoProductosTableProps) {
  return (
      ...
      {detalles && detalles.length > 0 ? (
        detalles.map((detalle, index) => {
          ...
          const nombreCompleto = nombreVariante 
            ...
            : nombreProducto;
          const unidad = detalle.productounidad?.unidad?.abreviatura || 'und';
...
// Pedido Total Card
interface PedidoTotalCardProps {
  total: number;
}

export function PedidoTotalCard({ total }: PedidoTotalCardProps) {
  return (
...
export function PedidoActionButtons({ onConfirmar, onCancelar }: PedidoActionButtonsProps) {
  return (
...
// Confirm Modal
interface ConfirmPedidoModalProps {
  visible: boolean;
  onClose: () => void;
  onConfirm: () => void;
}

export function ConfirmPedidoModal({ visible, onClose, onConfirm }: ConfirmPedidoModalProps) {
  return (
    <Modal
      visible={visible}
      animationType="fade"
...
// Cancel Modal
interface CancelPedidoModalProps {
  visible: boolean;
  onClose: () => void;
  onConfirm: () => void;
}

export function CancelPedidoModal({ visible, onClose, onConfirm }: CancelPedidoModalProps) {
  return (
    <Modal
      visible={visible}
      animationType="fade"
